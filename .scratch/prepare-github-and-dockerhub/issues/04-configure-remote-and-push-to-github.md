...
- [x] Configurar remote `origin` como `git@github.com:gugaucb/trade-simulator.git`.
- [x] Garantir que `.gitignore` exclui adequadamente arquivos sensíveis, bases SQLite locais e arquivos de log.
- [x] Confirmar que a chave SSH configurada no Ticket 01 autentica com sucesso no GitHub.
- [x] Realizar push da branch `main` e da branch `develop` para o GitHub (`git push -u origin main` e `git push -u origin develop`).
- [x] Validar visualização dos commits e arquivos na interface do repositório remoto.
